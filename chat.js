...
io.on('connection', function (socket) {
  ...
  if (socket.handshake.query.admin_id != undefined) {
    ...
    con.query(sql, (error, msg_rows) => {
        if (msg_rows) {
          ...
          con.query(login_status, (res_rows) => {
            if (res_rows) {
              response.push({ 'click_log_status': res_rows, 'each_msg': msg_rows });
              socket.emit('click_id_res', response);
              // console.log('empty');
    ...
    socket.on('chat_data', function (data) {
      ...
      data.forEach(function (chats) {
        ...
        // console.log(login_status);
        con.query(login_status, function (error, status) {
          if (!error) {
            ...
            con.query(insert_chat_data, function (error) {
              if (!error) {
      ...
      data.forEach(function (status) {
        ...
        con.query(sql, function (error, rows) {
          if (!error) {
            ...
            con.query(unread_msg, function (error, rows) {
                if (!error) {
    ...
    socket.on('click_id_msg_status', function (data) {
      ...
      data.forEach(function (status) {
        ...
        // console.log(update);
        con.query(update, function (error) {
          if (!error) {
            var sql = "select `user_id` from register_user where user_id != " + status.admin_id;
            // console.log(sql);
            con.query(sql, function (error, rows) {
    ...
    socket.on('header_menu', function (data) {
      if (data) {
        ...
        con.query(all_user, function (error, rows) {
          if (!error) {
            rows.forEach(function (data) {
              ...
              con.query(msg_count, function (error, res_data) {
  ...
  socket.on('disconnect', function () {
    if (socket.handshake.query.admin_id != undefined) {
      ...
      con.query(" update `register_user` set `user_status`=1 where `user_id`=" + socket.handshake.query.admin_id);
    }
  });
  ...
  function auto_logout(handshake_admin_id) {
    ...
    var s = new Date().getSeconds();
    var c_date = year + '-' + month + '-' + date + ' ' + h + ':' + m + ':' + s;
    con.query(" update `register_user` set `user_status`=0, `status_active_time`=" + "'" + c_date + "'" + " where `user_id`=" + socket.handshake.query.admin_id);
  }
  // end of disconnect
});
      ...
      if (file == undefined) {
        ...
        con.query(sql, (error) => {
          if (!error) {
           ...
           con.query(sql1, function (error, rows) {
              // res.redirect('/dashboard');
